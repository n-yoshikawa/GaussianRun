...
class GaussianDFTRun:
    ...
    def Extract_values(self, infilename, nmr, uv, energy, gap, dipole):
        ...
        if gap == 1:
            ...
            if BetaEigenVal == []:
                Alpha_gap = 27.211 * \
                    (AlphaEigenVal[NumAlphaElec] -
                     AlphaEigenVal[NumAlphaElec - 1])
            ...
            #    return Alpha_gap
                ...
                Alpha_gap = 27.211 * \
                    (AlphaEigenVal[NumAlphaElec] -
                     AlphaEigenVal[NumAlphaElec - 1])
                Beta_gap = 27.211 * \
                    (BetaEigenVal[NumBetaElec] - BetaEigenVal[NumBetaElec - 1])
        ...
        if dipole == 1:
            ...
            output["dipole"] = [Dipole_X[-1], Dipole_Y[-1],
                                Dipole_Z[-1], Dipole_Total[-1]]
...
# """
        ...
        subprocess.call(["g09", PreGauInput[0]])
...
###Calculating the total number of electrons#################
        ...
        for j in range(N):
            if (len(CHG_atom) != 0):
                Judge = CHG_atom - j
                if (any(Judge) == 0):
                    TotalNum_electron += AtomInfo.AtomicNumElec(
                        Mol_atom[j]) - CHG[where(Judge == 0)]
                else:
